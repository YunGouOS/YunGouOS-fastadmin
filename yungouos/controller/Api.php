...
use addons\yungouos\library\Wechat;
...
class Api extends Controller
{
    ...
    public function wechat()
    {
        ...
        if ($isWechat) {
            ...
            if ($sign != md5(implode('', $data) . $config['wechat']['key'])) {
                $this->error("签名不正确");
            }
            ...
            if ($this->request->isAjax()) {
                ...
                $result=$wxpay->getOrderInfoByOutTradeNo($out_trade_no,$mch_id,$key);
                if ($result['payStatus'] == 1) {
                    $this->success("", "", ['trade_state' => "SUCCESS"]);
                } else {
                    $this->error("查询失败");
                }
            }
    ...
    public function returnx()
    {
        ...
        $this->success("恭喜你！支付成功!", addon_url("yungouos/index/index"));
