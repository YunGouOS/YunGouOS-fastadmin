...
class WxPay
{
    ...
    {
        $this->apiConfig = require dirname(dirname(__FILE__)) . '/config/YunGouOSPayApiConfig.php';
    ...
    /**
     *  微信小程序支付，返回小程序支付所需的package 流程参考：https://open.pay.yungouos.com/#/api/api/pay/wxpay/minPay
     * @param $out_trade_no 订单号不可重复
     * @param $total_fee 支付金额 单位元 范围 0.01-99999
     * @param $mch_id 微信支付商户号 登录YunGouOS.com-》微信支付-》我的支付 查看商户号
     * @param $body  商品描述
     ...
     * @param $key 商户密钥 登录YunGouOS.com-》我的账户-》账户中心 查看密钥
     */
    public function minAppPay($out_trade_no, $total_fee, $mch_id, $body, $attach, $title,$notify_url, $key)
    {
        $result = null;
        $paramsArray = array();
        try {
            ...
            $resp = $this->httpUtil->httpsPost($this->apiConfig['minapp_pay_url'], $paramsArray);
            if (empty($resp)) {
                throw new Exception("API接口返回为空");
            }
            $ret = @json_decode($resp, true);
     ...
     * 验证回调签名
     */
    public function checkNotify($data, $key)
    {
        $paramsArray['code'] = $data['code'];
        $paramsArray['orderNo'] = $data['orderNo'];
        $paramsArray['outTradeNo'] = $data['outTradeNo'];
        $paramsArray['payNo'] = $data['payNo'];
        $paramsArray['money'] = $data['money'];
        $paramsArray['mchId'] = $data['mchId'];
        $reSign = $this->wxPaySign->getSign($paramsArray, $key);
        if ($reSign != $data['sign']) {
            return false;
        }
        return true;
